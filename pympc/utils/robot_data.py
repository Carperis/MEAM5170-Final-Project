...
class RobotData:
    ...
    def __init__(self, robot_model: str, state_estimation: bool | None = False) -> None:
    ...
    def update(
        self,
        pos_base: list | np.ndarray,
        lin_vel_base: list | np.ndarray,
        quat_base: list | np.ndarray,
        ang_vel_base: list | np.ndarray,
        q: list | np.ndarray,
        qdot: list | np.ndarray,
    ) -> None:
        if not self.state_estimation:
            self.pos_base: np.ndarray = vec_format_standardization(pos_base, "1darray")
            self.lin_vel_base: np.ndarray = vec_format_standardization(
                lin_vel_base, "1darray"
            )
            self.quat_base: np.ndarray = vec_format_standardization(
                quat_base, "1darray"
            )
            self.ang_vel_base: np.ndarray = vec_format_standardization(
                ang_vel_base, "1darray"
            )
        ...
        generalized_q = list(self.pos_base) + list(quat_base_converted) + list(self.q)
        self.__generalized_q = np.array(generalized_q, dtype=np.float32)
        pinocchio.forwardKinematics(
            self.__pin_model, self.__pin_data, self.__generalized_q
        )
        ...
        self.Jv_feet, self.base_Jv_feet = (
            self.__compute_foot_Jacobian()
        )  # geometric jacobian
    ...
    def __compute_foot_Jacobian(self) -> tuple[list[np.ndarray], list[np.ndarray]]:
        ...
        base_Jv_feet = []
        for i in range(4):
            Ji = pinocchio.computeFrameJacobian(
                self.__pin_model,
                self.__pin_data,
                self.__generalized_q,
                frame_id=foot_frames_ID[i],
    ...
    def __compute_base_vel_base_feet(self) -> list[np.ndarray]:
        ...
        for foot_idx in range(4):
            generalized_qdot = (
                list(self.lin_vel_base) + list(self.ang_vel_base) + list(self.qdot)
            )
    ...
    def __compute_pos_thighs(self) -> list[np.ndarray]:
        ...
        thigh_frames_ID = [
            self.__pin_model.getFrameId(thigh_frames[i]) for i in range(4)
        ]
        pos_thighs = []
    ...
    def __update_contact_history(self, cur_contact_state: list | np.ndarray) -> None:
        for foot_idx in range(4):
            if cur_contact_state[foot_idx] == 1:
                self.__contact_history[foot_idx] = self.pos_feet[foot_idx]

    def update_terrain_normal(self, cur_contact_state: list | np.ndarray) -> None:
