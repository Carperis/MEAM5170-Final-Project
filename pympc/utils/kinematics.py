...
def vec_format_standardization(
    vec: list | np.ndarray, std_type: str | None = "list"
) -> list | np.ndarray:
...
def quat2ZYXangle(quat: list | np.ndarray) -> list:
...
def quat2matrix(quat: list | np.ndarray) -> np.ndarray:
    q = vec_format_standardization(quat)
    assert len(q) == 4
...
def ZYXangles2matrix(ZYX_angle: list | np.ndarray) -> np.ndarray:
    ...
    r12 = math.cos(psi) * math.sin(theta) * math.sin(phi) - math.sin(psi) * math.cos(
        phi
    )
    r13 = math.cos(psi) * math.sin(theta) * math.cos(phi) + math.sin(psi) * math.sin(
        phi
    )
    r21 = math.sin(psi) * math.cos(theta)
    r22 = math.sin(psi) * math.sin(theta) * math.sin(phi) + math.cos(psi) * math.cos(
        phi
    )
    r23 = math.sin(psi) * math.sin(theta) * math.cos(phi) - math.cos(psi) * math.sin(
        phi
    )
    r31 = -math.sin(theta)
...
def matrix2quat(
    rotation_matrix: np.ndarray, quat_data_type: str | None = "list"
) -> list | np.ndarray:
    """
...
def vec2so3(vec: list | np.ndarray) -> np.ndarray:
...
def adSE3_Rp(R: np.ndarray, p: list | np.ndarray) -> np.ndarray:
...
def Rp2T(R: np.ndarray, p: list | np.ndarray) -> np.ndarray:
...
def exp_se3(screw_axes, theta) -> np.ndarray:
    ...
    if np.linalg.norm(omega) == 1:
        ...
        exp_se3[0:3, 3] = (
            theta * np.identity(3)
            + (1 - np.cos(theta)) * omega_ss
            + (theta - np.sin(theta)) * omega_ss @ omega_ss
        ) @ v
        exp_se3[3, 3] = 1
