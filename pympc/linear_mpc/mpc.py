...
class ModelPredictiveController:
    ...
    def update_mpc_if_needed(
        ...
        if iter_counter % self.iterations_between_mpc == 0:
            ref_traj = self.generate_reference_trajectory(
                vel_base_des, yaw_turn_rate_des
            )
            self.ref_traj = ref_traj
            solve_start = time.time()
            self.__contact_forces = self._solve_mpc(
            ...
            if debug and iter_counter == iter_debug:
                contact_forces_debug = self._solve_mpc(
                    ref_traj, gait_table, solver=solver, debug=debug
                )
    ...
    def generate_reference_trajectory(
        self, vel_base_des: list | np.ndarray, yaw_turn_rate: float
    ) -> np.ndarray:
        ...
        for i in range(1, self.horizon):
            X_ref[2 + self.num_state * i] = (
                X_ref[2 + self.num_state * (i - 1)] + self.dt * yaw_turn_rate
            )
            X_ref[3 + self.num_state * i] = (
                X_ref[3 + self.num_state * (i - 1)] + self.dt * vel_base_des[0]
            )
            X_ref[4 + self.num_state * i] = (
                X_ref[4 + self.num_state * (i - 1)] + self.dt * vel_base_des[1]
            )
    ...
    def _generate_state_space_model(self):
        ...
        for i in range(4):
            Bc[6:9, 3 * i : 3 * i + 3] = np.linalg.inv(world_I) @ vec2so3(
                self.pos_base_feet[i]
            )
    ...
    def _generate_QP_cost(self, Ad, Bd, xt, Xref, debug=False):
        ...
        Su = np.zeros(
            (self.num_state * self.horizon, self.num_input * self.horizon),
            dtype=np.float32,
        )
        ...
        for i in range(self.horizon):
            Sx[self.num_state * i : self.num_state * (i + 1), 0 : self.num_state] = (
                power_of_A[i + 1]
            )
            ...
            for j in range(self.horizon):
                if i >= j:
                    Su[
                        self.num_state * i : self.num_state * (i + 1),
                        self.num_input * j : self.num_input * (j + 1),
                    ] = (
                        power_of_A[i - j] @ Bd
                    )
    ...
    def _generate_QP_constraints(self, gait_table):
        ...
        qp_C = np.kron(
            np.identity(4 * self.horizon, dtype=np.float32), constraint_coef_matrix
        )
    ...
    def _solve_mpc(self, ref_traj, gait_table, solver="drake", debug=False):
        ...
        qpH, qpg = self._generate_QP_cost(
            Ad, Bd, self.current_state, ref_traj, debug=debug
        )
        # end_time = time.time()
        ...
        if solver == "drake":
            ...
            contact_forces = qp_problem.NewContinuousVariables(
                self.num_input * self.horizon, "contact_forces"
            )
            ...
            contact_forces = solve_qp(
                P=qpH, q=qpg, G=qp_C, h=C_ub, A=None, b=None, solver="osqp"
            )
