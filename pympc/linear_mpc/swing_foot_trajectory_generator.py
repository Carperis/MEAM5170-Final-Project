...
class SwingFootTrajectoryGenerator:
    ...
    def __init__(self, leg_id):
        ...
        self.__remaining_swing_time = 0.0
    ...
    def generate_swing_foot_trajectory(self, total_swing_time, cur_swing_time):
        break_points = np.array(
            [[0.0], [total_swing_time / 2.0], [total_swing_time]], dtype=np.float32
        )
    ...
    def compute_traj_swingfoot(self, robot_data: RobotData, gait: Gait):
        ...
        pos_swingfoot_des, vel_swingfoot_des = self.generate_swing_foot_trajectory(
            total_swing_time, cur_swing_time
        )
    ...
    def set_foot_placement(
        self, robot_data: RobotData, gait: Gait, base_vel_base_des, yaw_turn_rate_des
    ):
        ...
        world_footpos_final = (
            pos_base
            + R_base
            @ (pos_thigh_corrected + base_vel_base_des * self.__remaining_swing_time)
            + 0.5 * total_stance_time * vel_base
            + 0.03 * (vel_base - vel_base_des)
        )

        world_footpos_final[0] += (0.5 * pos_base[2] / self.__gravity) * (
            vel_base[1] * yaw_turn_rate_des
        )
        world_footpos_final[1] += (0.5 * pos_base[2] / self.__gravity) * (
            -vel_base[0] * yaw_turn_rate_des
        )
...
def test_swing_foot_traj():
    robot_path = os.path.join(
        os.path.dirname(__file__), "../robot/aliengo/urdf/aliengo.urdf"
    )
    ...
    test.set_foot_placement(robot_data, gait, np.array([0.5, 0.0, 0.0]), 0.0)
    base_pos_swingfoot_des, base_vel_swingfoot_des = test.compute_traj_swingfoot(
        robot_data, gait
    )
    print(base_pos_swingfoot_des, base_vel_swingfoot_des)

    test1 = SwingFootTrajectoryGenerator(1)
    test1.set_foot_placement(robot_data, gait, np.array([0.5, 0.0, 0.0]), 0.0)
    base_pos_swingfoot_des, base_vel_swingfoot_des = test1.compute_traj_swingfoot(
        robot_data, gait
    )
    print(base_pos_swingfoot_des, base_vel_swingfoot_des)

    test2 = SwingFootTrajectoryGenerator(2)
    test2.set_foot_placement(robot_data, gait, np.array([0.5, 0.0, 0.0]), 0.0)
    base_pos_swingfoot_des, base_vel_swingfoot_des = test2.compute_traj_swingfoot(
        robot_data, gait
    )
    print(base_pos_swingfoot_des, base_vel_swingfoot_des)

    test3 = SwingFootTrajectoryGenerator(3)
    test3.set_foot_placement(robot_data, gait, np.array([0.5, 0.0, 0.0]), 0.0)
    base_pos_swingfoot_des, base_vel_swingfoot_des = test3.compute_traj_swingfoot(
        robot_data, gait
    )
    print(base_pos_swingfoot_des, base_vel_swingfoot_des)
