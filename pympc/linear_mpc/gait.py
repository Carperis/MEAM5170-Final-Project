...
class Gait(Enum):
    ...
    def swing_time(self) -> float:
        return self.get_total_swing_time(
            self.__dt_control * self.__iterations_between_mpc
        )

    @property
    def stance_time(self) -> float:
        return self.get_total_stance_time(
            self.__dt_control * self.__iterations_between_mpc
        )

    def set_iteration(self, iterations_between_mpc: int, cur_iteration: int) -> None:
        self.iteration = (
            np.floor(cur_iteration / iterations_between_mpc) % self.num_segment
        )
    ...
    def get_swing_state(self) -> np.ndarray:
        swing_offsets_normalizerd = (
            self.stance_offsets_normalized + self.stance_durations_normalized
        )
        for i in range(4):
        ...
        phase_state = np.array(
            [self.phase, self.phase, self.phase, self.phase], dtype=np.float32
        )
